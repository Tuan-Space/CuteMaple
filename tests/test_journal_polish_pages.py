...
def test_structured_rows_keep_action_records(page):
    ...
    settle_queries();event=window.events_list.item(0);note=window.notes_list.item(0);day=window.day_items.item(0)
    settle_queries();assert event.data(Qt.UserRole)['id']==event_id
    settle_queries();assert event.data(ITEM_PRESENTATION_ROLE)['category']=='schedule'
    settle_queries();assert note.data(Qt.UserRole)['id']==note_id
    settle_queries();assert note.data(ITEM_PRESENTATION_ROLE)['title']=='今天的想法'
    settle_queries();assert day.data(Qt.UserRole)[1]['id']==event_id
    settle_queries();assert day.data(ITEM_PRESENTATION_ROLE)['title']=='明天一起散步'
    for widget in (window.events_list,window.notes_list,window.day_items,window.stat_list):
        settle_queries();assert isinstance(widget.itemDelegate(),WrappedItem)
...
def test_search_empty_state_is_not_an_actionable_record(page):
    ...
    for widget in (window.events_list,window.notes_list):
        item=widget.item(0)
        settle_queries();assert item.data(Qt.UserRole) is None
        settle_queries();assert item.data(ITEM_PRESENTATION_ROLE)['empty'] is True
        settle_queries();assert not item.flags() & Qt.ItemIsSelectable
    settle_queries();assert not window.note_restore.isEnabled()
...
def test_narrow_notes_use_full_content_width_and_restore(page):
    ...
    settle_queries();assert window.note_split.orientation()==Qt.Vertical
    settle_queries();assert window.notes_list.maximumHeight()==144
    settle_queries();assert window.note_editor.minimumHeight()>=window.note_editor.layout().totalMinimumHeightForWidth(window.note_editor.width())
    settle_queries();assert window.note_editor.pages.height()>=160
    window.resize(1000,700);app.processEvents()
    settle_queries();assert window.note_split.orientation()==Qt.Horizontal
    settle_queries();assert window.notes_list.maximumHeight()>1000


def test_settings_are_grouped_and_update_feedback_clears(page):
    _,_,window=page
    labels=window.tabs.widget(4).findChildren(QLabel)
    settle_queries();assert {'资料与备份','应用','录音'}<={label.text() for label in labels if label.objectName()=='section'}
    settle_queries();assert window.startup.text()=='开机自动启动'
    window.update_result({'new':True,'version':'99.0.0','body':'更新说明'})
    settle_queries();assert window.update_notes.text()=='更新说明' and not window.update_notes.isHidden()
    window.resize(620,420);QApplication.processEvents()
    settle_queries();assert window.nav[4].sizeHint().width()<=window.nav[4].width()
    settle_queries();assert '99.0.0' in window.nav[4].toolTip()
    window.update_result({'new':False})
    settle_queries();assert window.update_notes.isHidden() and window.nav[4].text()=='设置'
...
def test_unbroken_text_cannot_expand_settings_or_ledger_offscreen(page,monkeypatch):
    ...
    settle_queries();assert window.width()==620 and window.tabs.widget(4).horizontalScrollBar().maximum()==0
    store.save_event('W'*200,'todo','',Rule('2026-09-18T18:00:00'))
    seen=[]
    def inspect(dialog):
        dialog.show();app.processEvents()
        settle_queries();assert dialog.width()<=dialog.screen().availableGeometry().width()
        seen.append(dialog);dialog.reject();return QDialog.Rejected
    monkeypatch.setattr(QDialog,'exec',inspect)
    window.event_ledger(store.events()[0]);settle_queries();assert len(seen)==1
    ...
    for width in (620,800,1000):
        ...
        for mode in (0,1):
            ...
            for recording in (False,True):
                ...
                settle_queries();assert editor.pages.height()>=160
                settle_queries();assert editor.pages.geometry().bottom()<editor.files.geometry().top()
                settle_queries();assert editor.plus.mapTo(editor,editor.plus.rect().bottomRight()).y()<editor.pages.geometry().top()
                if recording:
                    settle_queries();assert editor.files.geometry().bottom()<editor.media.geometry().top()
...
def test_long_bubble_scrolls_without_hiding_response_actions(page):
    ...
    settle_queries();assert bubble.pet._screen_area().contains(bubble.geometry())
    settle_queries();assert bubble.content_area.verticalScrollBar().maximum()>0
    settle_queries();assert bubble.text.height()>=bubble.text.heightForWidth(bubble.text.width())
    settle_queries();assert bubble.done.geometry().bottom()<bubble.height()
