...
def test_batches_no_dead_pages(view,count):
    ...
    w.refresh_events();settle_queries();assert w.event_more==(count>50)
    settle_queries();assert len([w.events_list.item(i) for i in range(w.events_list.count()) if w.events_list.item(i).data(Qt.UserRole)])==min(count,50)
    w.tabs.setCurrentIndex(2);a.processEvents();settle_queries();assert w.note_more==(count>50)
    ...
    settle_queries();assert w.notes_list.count()==max(count,1)
    settle_queries();assert w.note_editor.title.text()=='没有保存的内容'
    settle_queries();assert not any(b.text() in ('上一页','下一页') for b in w.findChildren(QPushButton))
    w.note_search.setText('找不到的关键词');a.processEvents();settle_queries();assert w.notes_list.count()==1;settle_queries();assert not w.note_more
    settle_queries();assert not w.note_delete.isEnabled()
...
def test_calendar_all_types_and_preserved_history(view):
    ...
    settle_queries();assert w.calendar.marks['2026-09-17']=={'todo','schedule','anniversary','note','habit'}
    settle_queries();assert any(row['title']=='todo' for kind,row,_ in w.calendar_items['2026-09-17'] if kind=='todo')
    w.calendar_filter.setCurrentIndex(4);settle_queries();assert w.calendar.marks['2026-09-17']=={'note'}
    settle_queries();assert len(w.calendar_items['2026-09-17'])==2
    s.trash_note(s.notes()[0]['id']);w.refresh_calendar();settle_queries();assert len(w.calendar_items['2026-09-17'])==1
...
def test_theme_and_milestone_form(view,monkeypatch):
    ...
    settle_queries();assert w._theme['background']=='#202020';settle_queries();assert w.legend.text()!=old
    d=EventEditor(s,parent=w,initial_kind='anniversary');d.show();a.processEvents()
    settle_queries();assert d.is_milestone();settle_queries();assert not d.advanced.isVisible();settle_queries();assert d.start.time().hour()==10
    ...
    r=parse_rule(s.events()[0]['rule']);settle_queries();assert isinstance(r,MilestoneRule);settle_queries();assert r.days==(30,520,1000)
    d.close()
...
def test_single_bubble_hides_navigation_and_keeps_write_error(view,monkeypatch):
    ...
    settle_queries();assert not b.previous.isVisible() and not b.following.isVisible() and not b.count.isVisible()
    def fail(*args):raise OSError('模拟写入失败')
    monkeypatch.setattr(s,'respond',fail);b.respond('done')
    b.refresh();settle_queries();assert b.meta.isVisible() and '未能保存' in b.meta.text();settle_queries();assert b.isVisible();b.close()
...
def test_archiving_cancels_future_calendar_entry(view):
    ...
    s.tick();settle_queries();assert s.pending();s.archive_event(identity)
    w.calendar.setSelectedDate(QDate(2026,9,18));w.refresh_calendar()
    settle_queries();assert not w.calendar_items.get('2026-09-18')
