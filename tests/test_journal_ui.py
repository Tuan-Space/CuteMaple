...
def test_sleep_audio_even_when_reactions_disabled(monkeypatch):
    ...
    p._audio_activity(True);p._check_system_idle();assert p.sleep_phase is None
    p._audio_activity(False);p._check_system_idle();assert p.sleep_phase is None
    p.sleep_phase='loop';p._audio_activity(True);assert p.sleep_phase=='exit'
    p.tray.hide();p.close()
...
def test_double_click_only_opens_journal(monkeypatch):
    ...
    p.mouseDoubleClickEvent(e);assert opened==[True];assert p.suppress_release_click
    p.tray.hide();p.close()
...
def test_bubble_persistent_hidden_and_clickable(tmp_path):
    ...
    assert b.isVisible();assert not b.testAttribute(Qt.WA_TransparentForMouseEvents)
    assert Pet()._screen_area().contains(b.geometry())
    old=s.pending()[0]['id'];b.refresh();assert s.pending()[0]['id']==old
    b.respond('done');assert not b.isVisible();b.close();s.close()
