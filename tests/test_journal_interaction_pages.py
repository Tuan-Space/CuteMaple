...
def trash_notes(store,w,count=105):
    ...
    w.tabs.setCurrentIndex(2);w.trash.setChecked(True);w.reset_notes();QApplication.processEvents()
    return ids
...
def test_right_checks_do_not_change_preview_and_space_toggles(page):
    ...
    assert w.note_editor.identity==identity
    assert w.note_checked=={second.data(Qt.UserRole)['id']}
    w.notes_list.setCurrentItem(second);QTest.keyClick(w.notes_list,Qt.Key_Space)
    assert not w.note_checked and w.note_editor.identity==identity
    ...
    assert w.note_editor.identity==second.data(Qt.UserRole)['id'] and not w.note_checked
    assert w.note_editor.read_only and w.note_editor.isEnabled()


def test_all_results_include_unloaded_pages_and_filter_or_exit_clears(page):
    _,store,w=page;ids=trash_notes(store,w)
    assert w.notes_list.count()==50
    w.select_all_notes(True);assert w.note_checked==set(ids)
    w.note_limit=100;w.refresh_notes();assert len(w.note_checked)==105
    assert all(w.notes_list.item(n).checkState()==Qt.Checked for n in range(100))
    w.note_search.setText('记录 00');assert not w.note_checked
    w.select_all_notes(True);assert len(w.note_checked)==10
    w.tabs.setCurrentIndex(0);assert not w.note_checked
    w.tabs.setCurrentIndex(2);assert not w.note_restore.isEnabled()
...
def test_empty_trash_has_no_checkboxes_or_mutation_tools(page):
    ...
    assert w.notes_list.item(0).data(Qt.CheckStateRole) is None
    assert not w.note_restore.isEnabled() and not w.note_purge.isEnabled()
    assert w.note_new.isHidden() and w.note_editor.format_toolbar.isHidden()
...
def test_unchecked_preview_never_becomes_bulk_target_and_cancel_preserves_checks(page,monkeypatch):
    ...
    w.restore_checked_notes();assert len(store.notes(trash=True))==3
    w.select_all_notes(True);monkeypatch.setattr(QMessageBox,'question',lambda *args:QMessageBox.No)
    w.purge_checked_notes();assert w.note_checked==set(ids) and len(store.notes(trash=True))==3
    monkeypatch.setattr(QMessageBox,'question',lambda *args:QMessageBox.Yes)
    w.purge_note(w.notes_list.item(1).data(Qt.UserRole))
    assert len(store.notes(trash=True))==2 and len(w.note_checked)==2
...
def test_delete_current_flushes_then_clears_identity_and_cannot_recreate(page,monkeypatch):
    ...
    assert w.note_editor.identity is None and not store.rows('SELECT * FROM notes WHERE id=?',(identity,))
    assert not w.note_editor.timer.isActive()
    ...
    with sqlite3.connect(store.root/'journal.sqlite3') as db:
        assert db.execute('SELECT COUNT(*) FROM notes WHERE id=?',(identity,)).fetchone()[0]==0
...
def test_delete_other_note_does_not_finish_current_edit_and_failure_blocks_delete(page,monkeypatch):
    ...
    w.purge_note_identity(other);assert not calls and w.note_editor.identity==current
    w.purge_note_identity(current);assert calls and store.rows('SELECT id FROM notes WHERE id=?',(current,))
...
def test_reminder_bulk_retains_expired_and_uses_full_selection(page):
    ...
    assert w.pending_reminders==[('event',expired)]
    assert w.event_checked=={('event',expired)}
    assert not w.event_bulk.attention.isHidden()
    assert store.rows('SELECT deleted FROM events WHERE id=?',(expired,))[0]['deleted'] is not None
    assert store.rows('SELECT deleted FROM events WHERE id=?',(future,))[0]['deleted'] is None
    w.event_search.setText('过期');assert not w.event_checked


def test_health_success_is_silent_and_today_visibly_actionable(page):
    _,_,w=page;w.status.hide();assert w.save_habit('water')
    assert w.status.isHidden() and w.calendar.today.objectName()=='secondary'
...
def test_calendar_open_from_trash_loads_requested_active_note_without_unlocking_trash(page):
    ...
    assert w.note_editor.read_only
    w.tabs.setCurrentIndex(0);row=store.rows('SELECT * FROM notes WHERE id=?',(identity,))[0]
    w.open_calendar_value('note',row)
    assert w.note_editor.identity==identity and w.note_editor.title.text()=='总览目标'
    assert w.note_editor.preview.toPlainText()=='正常正文'
    assert not w.note_editor.read_only and not w.trash.isChecked() and not w._notes_trash_state
    assert w.page_title.text()=='笔记' and not w.note_checked
...
def test_body_has_no_hidden_native_checkbox_hitbox(page):
    ...
    assert item.checkState()==Qt.Unchecked and not w.note_checked
    assert w.note_editor.identity==item.data(Qt.UserRole)['id']
...
def test_pending_finite_reminder_entry_requires_rescheduling(page,monkeypatch):
    ...
    assert seen and seen[0]['read_only'] is False
    assert store.rows('SELECT deleted FROM events WHERE id=?',(identity,))[0]['deleted'] is not None
