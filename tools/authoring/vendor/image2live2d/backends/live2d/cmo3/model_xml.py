...
# The four IR keyform helpers are extracted from the pinned upstream source.
# No binary MOC3 writer is included in this authoring-only subset.
from .irr_helpers import _affecting_params, _offset_at, _opacity_at, _opacity_params
...
def build_main_xml(
    ...
    # Allocate all drawable identities before meshes so forward clipping
    # references resolve regardless of their visual stacking order.
    drawable_guids = {part.id: sh.add("CDrawableGuid", uuid=_new_uuid(), note=part.id)[1]
                      for part in drawn}
    ...
    for i, part in enumerate(drawn):
        ...
        ref_drawable = drawable_guids[part.id]
        ...
        ref_mesh = _build_art_mesh(
            ...
            ref_tex2d, [drawable_guids[target] for target in part.clip_to], part.invert_clipping)
...
def _mesh_keyform_grid(sh, rig, part, mesh, canvas_w, canvas_h, pidx, param_guid_ref):
    ...
    order_only = [p for p in rig.parameters if p not in affecting and p not in opac_only
                  and any(part_id in k.draw_order_overrides for k in p.keyforms)]
    affecting = sorted(affecting + opac_only + order_only, key=lambda p: pidx[p.id])

    if not affecting:
        ref_form = sh.add("CFormGuid", uuid=_new_uuid(), note=f"{part_id}_rest")[1]
        return _static_keyform_grid(sh, ref_form), [(ref_form, rest_px, part.opacity, part.draw_order)]
    ...
    for ref_form, axis_vals in cells:
        ...
        for p, val in zip(affecting, axis_vals):
            ...
            if ov is not None:
                op *= ov
            keyed = next((k.draw_order_overrides[part_id] for k in p.keyforms
                          if abs(k.value-val)<1e-7 and part_id in k.draw_order_overrides), None)
            if keyed is not None:
                order, order_drivers = keyed, order_drivers+1
        if order_drivers>1:
            raise ValueError(f"Conflicting draw-order drivers on {part_id}")
        cell_forms.append((ref_form, [(x * canvas_w, y * canvas_h) for x, y in pos], op, order))
    ...
    for p in rig.parameters:
        ...
        for kf in p.keyforms:
            for dx, dy in kf.deformer_offsets.get(def_id, []):
                mag += abs(dx) + abs(dy)
        if mag > 1e-9 or any(def_id in k.deformer_opacity_overrides or def_id in k.deformer_offset_weights for k in p.keyforms):
            out.append(p)
...
def _deformer_form_super(parent, tag, ref_form, ref_source, ref_coord, opacity=1.):
    ...
    _text(adf, "f", f"{opacity:.6f}", xs__n="opacity")
...
def _build_warp_deformer(sh, rig, d, cw, ch, ref_coord, param_guid_ref, pidx, ref_part_root,
        ...
        for ref_form, axis_vals in cells:
            ...
            for p, val in zip(affecting, axis_vals):
                for j, (dx, dy) in enumerate(_deformer_offset_at(p, val, d.id, npts)):
                    pts[j][0] += dx
                    pts[j][1] += dy
                keyed=next(k for k in p.keyforms if k.value==val)
                opacity*=keyed.deformer_opacity_overrides.get(d.id,1.)
                offset_weight*=keyed.deformer_offset_weights.get(d.id,1.)
            pts=[(bx+(x-bx)*offset_weight,by+(y-by)*offset_weight)
                 for (bx,by),(x,y) in zip(base,pts)]
            cell_forms.append((ref_form, [(x * cw, y * ch) for x, y in pts],opacity))
    else:
        ref_form = sh.add("CFormGuid", uuid=_new_uuid(), note=f"{d.id}_rest")[1]
        ref_grid = _static_keyform_grid(sh, ref_form)
        cell_forms = [(ref_form, [(x * cw, y * ch) for x, y in base],1.)]
    ...
    for ref_form, grid_px, opacity in cell_forms:
        form = _sub(kf_list, "CWarpDeformerForm")
        _deformer_form_super(form, "CWarpDeformerSource", ref_form, ref_src, ref_coord,opacity)
...
def _build_art_mesh(
    ...
    clipping_refs=(), invert_clipping=False,
) -> str:
    ...
    clips = _sub(ds, "carray_list", xs__n="clipGuidList", count=str(len(clipping_refs)))
    for reference in clipping_refs:
        _sub(clips, "CDrawableGuid", xs__ref=reference)
    _text(ds, "b", "true" if invert_clipping else "false", xs__n="invertClippingMask")
    ...
    for ref_form, cell_px, cell_opacity, cell_order in cell_forms:
        ...
        _sub(acf, "null", xs__n="name")
        _text(acf, "s", "", xs__n="notes")
        _text(adf, "i", str(cell_order), xs__n="drawOrder")
...
def _param_source(parent, param, ref_param_group, ref_guid) -> None:
    ...
    # Cubism's ordinary parameter interpolation uses 1.5 * snapEpsilon.
    # A fixed 0.1 is therefore unsafe for normalized parameters, especially
    # distinct material guard keys only 0.0001 apart. Keep the interpolation
    # tolerance well below every serialized key gap instead of changing keys.
    keys = sorted(set(float(f"{k.value:.4f}") for k in param.keyforms))
    if len(keys) != len(param.keyforms):
    ...
    _text(ps, "f", f"{snap_epsilon:.9g}", xs__n="snapEpsilon")
