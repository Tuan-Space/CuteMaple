...
class Part(BaseModel):
    ...
    clip_to: list[str] = Field(default_factory=list)
    invert_clipping: bool = False
...
class Keyform(BaseModel):
    ...
    * ``opacity_overrides[part_id]`` -> absolute opacity at this keyform.
    ...
    deformer_opacity_overrides: dict[str, float] = Field(default_factory=dict)
    # Authoring-only multiplier of the summed coordinate offsets. The CMO3
    # backend expands this into ordinary multi-parameter keyform positions.
    deformer_offset_weights: dict[str, float] = Field(default_factory=dict)
    opacity_overrides: dict[str, float] = Field(default_factory=dict)
    draw_order_overrides: dict[str, int] = Field(default_factory=dict)
...
class Rig(BaseModel):
    ...
    def _integrity(self) -> "Rig":
        ...
        for p in self.parts:
            ...
            if any(target not in part_ids or target == p.id for target in p.clip_to):
                raise ValueError(f"part {p.id!r} has missing/self clipping source")
        ...
        for param in self.parameters:
            for kf in param.keyforms:
                ...
                for did,value in kf.deformer_opacity_overrides.items():
                    if did not in def_ids or not 0<=value<=1:
                        raise ValueError(f"{param.id!r} invalid deformer opacity {did!r}")
                for did,value in kf.deformer_offset_weights.items():
                    if did not in def_ids or not 0<=value<=1:
                        raise ValueError(f"{param.id!r} invalid authoring offset weight {did!r}")
                for pid in kf.opacity_overrides:
                    if pid not in part_ids:
                        raise ValueError(f"{param.id!r} opacity override unknown part {pid!r}")
                for pid in kf.draw_order_overrides:
                    if pid not in part_ids:
                        raise ValueError(f"{param.id!r} draw order override unknown part {pid!r}")
