"""Dense, deterministic screenshot audit in an isolated library; no microphone use."""
...
def run():
    ...
    note()
    report={'theme':a.theme,'scale':os.environ.get('QT_SCALE_FACTOR','1'),'fonts':load_journal_fonts(),'fixedTime':fixed.isoformat(),'recordingStatesSimulated':True,'captures':[],'horizontalOverflow':{}}
    def wait(ms):
    ...
    if a.extended:
        ...
        d=EventEditor(s,parent=w);d.resize(560,620);d.period.setCurrentIndex(4);d.advanced.show();d.show();capture('event-advanced',d)
        d.calendar.setCurrentIndex(1);d.start.setDateTime(QDateTime(datetime(2025,7,25,9)));capture('event-leap-month',d);d.custom_days.setText('不合法');d.kind.setCurrentIndex(2);capture('event-validation-error',d);d.close()
        class Pet:
            _pet_hidden=True
